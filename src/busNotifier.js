    ...
    if (table.wakeWords.includes(wakeWord)) {
      ...
      table.timeTable.forEach(bus => {
        ...
        if (moment(next).isBetween(moment(), moment().add(60, "m"))) {
          results.nexts.push({ "time": moment(next).format("HH:mm"), "mins": moment(next).diff(moment(), "minutes"), "type": bus.type })
        }
      })
    }
  })
...
const createMessage = ({ data }) => {
  ...
  data.nexts.forEach((bus, index) => {
    text += `${index + 1}：${bus.type ? bus.type === "exp" ? "急行" : "各停" : ""} ${bus.time.toString()}発 - ${bus.mins}分後\n`
  })
